...
Historical documents must not be merged into the patient's working record until their identity has been provisionally resolved.
#### Frontend / Workflow
...
The original free-text term must remain available alongside its mapped code.
#### Backend / EHR
...
Recommendations must remain suggestions and require explicit doctor action. They must never be automatically applied to the record.
#### Backend / EHR
...
The project requires NER performance to be measured against a clinically reviewed test set.
#### Backend / EHR Testing
...
* Audit logging
* End-to-end latency
Measure false-match and false-non-match rates separately for patient matching.
