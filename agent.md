# Agent Instructions

**CRITICAL:** `project_docs/EHR_Architecture_Document.md` is the absolute source of truth for the architecture of this platform. It is not to be messed with, altered, or contradicted. All design decisions, code implementation, and architectural boundaries must strictly adhere to the guidelines set in that document. All project documentation, scope specifications, and timelines are located in the `project_docs/` directory:
- `project_docs/EHR_Architecture_Document.md` (System Architecture & Reference Architecture)
- `project_docs/EHR_Clinical_NLP_HL7_Project_Scope (1).md` (Project Scope, Flow, and Acceptance Criteria)
- `project_docs/EHR_Timeline_and_Milestones.md` (8-Week Implementation Timeline & Milestones)

**LOGGING WORK:** All agents and developers MUST log their completed work in `progress.md`. Whenever you complete a significant chunk of work, prepend a new entry to the top of the log (below the header) so that the team can track what has been done and what remains.

**GIT COMMITS & PUSHES:** Agents and developers MUST ALWAYS ASK for explicit confirmation from the user BEFORE committing or pushing any changes into the GitHub repository. Never commit or push automatically without user permission.

